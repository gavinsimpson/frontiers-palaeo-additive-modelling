...
library("tidyr")
```
In addition, the *gratia* package is required; this in-development package is not on CRAN but can be installed directly from GitHub using functions from the *devtools* package. To install the package, install *devtools* and then use `devtools::install_github()` to install *gratia*, as shown below:
...
The same model as used for Small Water was also initially attempted with the Braya-Sø data. However, in order to even fit the model with both a smooth and the CAR(1) process, I had to change the default optimizer used to estimate the model parameters, and reduce the basis dimension, `k`, to a small number. I also fit the model using GCV, which is, at the time of writing, the default in `gam()`, hence no `method` argument
